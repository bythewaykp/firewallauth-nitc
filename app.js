...
(async () => {
  ...
  do{
    ...
    if(a==s){
      ...
      data= `${un},${dateformater(pd)},${name}\n`

      fs.appendFile('test/s.csv',data,(e)=>{})

        await page.goto("http://172.16.32.1:1000/logout?",{
            waitUntil: 'networkidle2'
        })
      await page.click('body > div > div > form > p:nth-child(5) > a')
        ...
        data = `${un},${dateformater(pd)},${name}\n`
      fs.appendFile('test/c.csv',data,(e)=>{})
